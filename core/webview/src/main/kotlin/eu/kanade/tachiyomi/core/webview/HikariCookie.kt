...
interface HikariCookieDao {
    ...
    @Upsert
    suspend fun upsert(cookie: HikariCookieEntity)

    @Query("DELETE FROM hikari_cookies WHERE domain = :domain AND name = :name AND sourceId = :sourceId")
    suspend fun delete(domain: String, name: String, sourceId: Long)
...
@Singleton
class HikariCookieStorage @Inject constructor(
    ...
    suspend fun purgeExpired() = dao.purgeExpired()
